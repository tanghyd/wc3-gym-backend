...
- **Production is Portainer only.** The maintainer has no shell on the VM. Every production step in these pages is a Portainer action; the dump and restore run as one-off containers from `ghcr.io/tanghyd/gnl-db-backup` (source `deploy/backup/`). The stack file itself is not in any repository yet; copying it out of Portainer into `deploy/` is the first thing to do.
- **Staging deploy access.** The staging box is Daniel's and is deployed over SSH with the `just` recipes in his workspace. Those recipes are not in the repositories on purpose: they do not match how production is deployed. Running Portainer on staging too would make the two the same.
- **Automation.** A Portainer stack webhook called from GitHub Actions would deploy with no SSH and no IP allowlist (DEPLOYMENT.md, "Automating it later"). EAShibby's position is that a manual, well-documented path must stay available when the person who built the automation is gone; the webhook and the manual path run the same stack file, so both hold.
