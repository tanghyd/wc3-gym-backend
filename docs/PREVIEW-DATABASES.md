...
The database name is `wc3gym_<slug>_<hash>`: the slug is the branch name lower-cased with runs of non-alphanumerics folded to `_` and cut to 30 characters, for reading; the hash is the first 8 hex digits of the sha1 of the exact branch name, so `feature/foo-bar` and `feature/foo_bar` never share a database. 46 characters at most, inside Postgres's 63-byte identifier limit. The branch name, not the pull request number, is the key: Vercel builds a branch as soon as it is pushed, which is before its pull request exists, and does not rebuild when the pull request opens.
