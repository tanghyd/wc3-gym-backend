...
class DBUser(DBModel):
    ...
    team_seasons: Mapped[list['DBUserTeamSeason']] = relationship(back_populates='user', cascade="all, delete")
    w3c_stats: Mapped[list['DBW3CStats']] = relationship(back_populates='user', cascade='all, delete-orphan')
    fantasy_teams: Mapped[list['DBFantasyTeamPlayer']] = relationship(back_populates='users', cascade='all, delete-orphan')
    signup_seasons: Mapped[list['DBUserSeasonSignup']] = relationship(back_populates='user', cascade="all, delete")
    career_stats: Mapped[list['DBPlayerCareerStats']] = relationship(back_populates='user')
    ...
    def updateUserTeamSeasonStats(cls, session: Session, season_stats):
        ...
        uts_obj = session.get(DBUserTeamSeason, {'team_id': team.id, 'season_id': season.id, 'user_id': user.id})
