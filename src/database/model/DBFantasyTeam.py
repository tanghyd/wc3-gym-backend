...
class DBFantasyTeam(DBModel):
    ...
    drafted_team_id: Mapped[int | None] = mapped_column(ForeignKey('teams.id', ondelete='CASCADE'))
    ...
    drafted_players: Mapped[list['DBFantasyTeamPlayer']] = relationship(back_populates='fantasy_team', cascade="all, delete")
    ...
    def addPlayers(cls, session: Session, obj_id, user_ids):
        ...
        for user_id in user_ids:
            ...
            already_exists = session.get(DBFantasyTeamPlayer, {'fantasy_team_id': team.id, 'user_id': user.id}) is not None
    ...
    def removePlayers(cls, session: Session, obj_id, user_ids):
        ...
        for user_id in user_ids:
            ...
            if not user:
                raise Exception(f"User not found by id: {user_id}")
            user_team = session.get(DBFantasyTeamPlayer, {'fantasy_team_id': obj_id, 'user_id': user.id})
            if not user_team:
