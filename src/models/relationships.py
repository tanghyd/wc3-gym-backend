...
class DBUserTeamSeason(DBModel):
    ...
    matchup_history: Mapped[list | None] = mapped_column(JSON)  # Array of opponent races: ['HU', 'OC', 'UD', etc.]
...
class DBFantasyTeamPlayer(DBModel):
    ...
    fantasy_team_id: Mapped[int] = mapped_column(ForeignKey('fantasy_teams.id'), primary_key=True)
    user_id: Mapped[int] = mapped_column(ForeignKey('users.id'), primary_key=True)
    # Additional columns can be added here if needed
    fantasy_team: Mapped['DBFantasyTeam'] = relationship(back_populates='drafted_players')
