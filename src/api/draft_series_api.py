...
def add_draft_series():
    try:
        data = request.json
        draft_series = (
            draft_series_blueprint.draft_series_app_service.create_draft_series(
                DraftSeries(data)
            )
        )
        if draft_series:
...
def update_draft_series(draft_series_id):
    try:
        data = request.json
        draft_series = (
            draft_series_blueprint.draft_series_app_service.update_draft_series(
                draft_series_id, DraftSeries(data)
            )
        )
        if draft_series:
            draft_series = draft_series.to_dict()
...
def delete_draft_series(draft_series_id):
    try:
        draft_series_blueprint.draft_series_app_service.delete_draft_series(
            draft_series_id
        )
...
def get_draft_series(draft_series_id):
    try:
        draft_series = draft_series_blueprint.draft_series_app_service.get_draft_series(
            draft_series_id
        )
        if draft_series:
            draft_series = draft_series.to_dict()
...
def get_draft_series_by_match(match_id: int):
    try:
        draft_series_list = (
            draft_series_blueprint.draft_series_app_service.get_draft_series_by_match(
                match_id
            )
        )
        out = []
...
def delete_all_draft_series_for_match(match_id: int):
    try:
        draft_series_blueprint.draft_series_app_service.delete_all_drafts_for_match(
            match_id
        )
...
@draft_series_blueprint.route(
    "/draft-series/<int:draft_series_id>/promote", methods=["POST"]
)
@jwt_required()
@swag_from(
    {
...
def promote_draft_series(draft_series_id):
    try:
        ...
        series_dto = draft_series_blueprint.draft_series_app_service.convert_to_series(
            draft_series
        )
