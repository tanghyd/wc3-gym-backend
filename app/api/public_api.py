        ...
        "parameters": [
            {
                ...
                "schema": {
                    ...
                    "properties": {
                        "client_token": {
                            "type": "string",
                            "description": "Shared client token between bot and backend",
                        },
                        "discord_id": {
                            "type": "string",
                            "description": "Discord user id to prefill",
                        },
                        "discord_tag": {
                            "type": "string",
                            "description": "Discord user tag (name#discriminator)",
                        },
                        "season_id": {
                            "type": "string",
                        ...
                        "access_type": {
                            "type": "string",
                            "description": "Type of access: signup, dashboard, or fantasy",
                            "enum": ["signup", "dashboard", "fantasy"],
                        },
                        "ttl_minutes": {
                            "type": "integer",
                            "description": "Token TTL in minutes (optional, default 30)",
                        },
                    },
                    "required": [
                        "client_token",
...
def create_public_access_helper():
    try:
        ...
        ttl_minutes = int(
            data.get("ttl_minutes") or request.args.get("ttl_minutes") or 30
        )
...
def public_create_user():
    ...
    try:
        ...
        if hasattr(public_api_blueprint, "settings_app_service"):
            try:
                signup_enabled = public_api_blueprint.settings_app_service.get_setting(
                    "signups_enabled"
                )
                if (
                    signup_enabled
                    and signup_enabled.get("value", "true").lower() == "false"
                ):
                    return jsonify(
                        {
        ...
        if not public_api_blueprint.user_app_service.validateBattleTag(
            user_payload["battleTag"]
        ):
            return jsonify(
                {
        ...
        try:
            ...
            return jsonify(
                {"error": "Error searching for existing user by discord id"}
            ), 500
        ...
        if existing_users and len(existing_users) > 0:
            ...
            try:
                ...
                user = public_api_blueprint.user_app_service.update_user(
                    existing.id, user_dto
                )
        ...
        season_id = (
            entry.get("season_id") or data.get("season_id") or data.get("seasonId")
        )
        if season_id and hasattr(public_api_blueprint, "season_app_service"):
            try:
                public_api_blueprint.season_app_service.addUserSignup(
                    int(season_id), [user.id]
                )
    ...
    try:
        ...
        try:
            ...
            if entry.get("season_id"):
                ...
                series = public_api_blueprint.series_app_service.searchForSeason(
                    entry.get("season_id"), query
                )
            else:
    ...
    try:
        ...
        try:
            ...
            for file_key in ["game1", "game2", "game3"]:
                if file_key in files and files[file_key].filename:
                    ...
                    if file and allowed_file(file.filename):
                        ...
                        logger.info(
                            f"Prepared replay file for Discord: {file.filename}"
                        )
                    else:
                        logger.warning(
                            f"File {file_key} failed validation: {file.filename}"
                        )
                        return jsonify(
                            {
            ...
            if action == "score_updated":
                logger.info(
                    "Processing as score update; enforcing replay upload requirements"
                )
            ...
            # Update the series
            updated_series = public_api_blueprint.series_app_service.update_series(
                series_id, series
            )
            ...
            if uploaded_files:
                result["uploaded_files"] = {
                    k: v["filename"] for k, v in uploaded_files.items()
                }
        ...
        "parameters": [
            {
                ...
                "schema": {
                    ...
                    "properties": {
                        ...
                        "user_name": {
                            "type": "string",
                            "description": "Name for new user creation if needed",
                        },
                        "battle_tag": {
                            "type": "string",
                            "description": "BattleTag for new user creation if needed",
                        },
                    },
                    "required": [
                        "token",
...
def create_fantasy_team():
    ...
    try:
        ...
        if hasattr(public_api_blueprint, "settings_app_service"):
            try:
                fantasy_enabled = public_api_blueprint.settings_app_service.get_setting(
                    "fantasy_team_creation_enabled"
                )
                if (
                    fantasy_enabled
                    and fantasy_enabled.get("value", "true").lower() == "false"
                ):
                    return jsonify(
                        {
                ...
                logger.warning(
                    f"Could not check fantasy_team_creation_enabled setting: {e}"
                )
                # Continue if setting doesn't exist
        ...
        if not hasattr(public_api_blueprint, "user_app_service") or not hasattr(
            public_api_blueprint, "fantasy_team_app_service"
        ):
        ...
        try:
            ...
            if not users or len(users) == 0:
                ...
                user = public_api_blueprint.user_app_service.create_user(
                    User(user_payload)
                )
        ...
        try:
            ...
            existing_teams = (
                public_api_blueprint.fantasy_team_app_service.search_fantasy_teams(
                    team_query
                )
            )
            ...
            if existing_teams and len(existing_teams) > 0:
                ...
                team = (
                    public_api_blueprint.fantasy_team_app_service.update_fantasy_team(
                ...
                # Create new team
                team = (
                    public_api_blueprint.fantasy_team_app_service.create_fantasy_team(
            ...
            if player_ids and len(player_ids) > 0:
                ...
                players_to_add = [
                    pid for pid in player_ids if pid not in existing_player_ids
                ]
                players_to_remove = [
                ...
                if players_to_add:
                    public_api_blueprint.fantasy_team_app_service.addFantasyPlayers(
                        team_id, players_to_add
                    )
                if players_to_remove:
                    public_api_blueprint.fantasy_team_app_service.removeFantasyPlayers(
                        team_id, players_to_remove
                    )

            # Return created/updated team
            final_team = public_api_blueprint.fantasy_team_app_service.get_fantasy_team(
                team_id
            )
            return jsonify(
                final_team.to_dict() if hasattr(final_team, "to_dict") else final_team
            ), 201
        ...
        "parameters": [
            {
                ...
                "schema": {
                    ...
                    "properties": {
                        ...
                        "series_id": {
                            "type": "integer",
                            "description": "ID of the series to bet on",
                        },
                        "season_id": {
                            "type": "integer",
                        ...
                        "winner_id": {
                            "type": "integer",
                            "description": "ID of the predicted winner",
                        },
                        "bet_points": {
                            "type": "integer",
                            "description": "Points to bet (required if not using fixed points)",
                        },
                    },
...
def create_fantasy_bet():
    ...
    try:
        ...
        bet = public_api_blueprint.fantasy_bet_app_service.create_fantasy_bet(
            FantasyBet(bet_payload)
        )
...
def update_fantasy_bet(bet_id):
    ...
    try:
        ...
        existing_bet = public_api_blueprint.fantasy_bet_app_service.get_fantasy_bet(
            bet_id
        )
        if not existing_bet:
        ...
        bet = public_api_blueprint.fantasy_bet_app_service.update_fantasy_bet(
            bet_id, FantasyBet(bet_payload)
        )
